...
public class Health : MonoBehaviour {
	...
	void Start () {
		spacingX = heartGUI.pixelInset.width/2;
		spacingY = -heartGUI.pixelInset.height/2;
	...
	public void AddHearts(int n) {
		for (int i = 0; i < n; i ++) { 
			...
			newHeart.GetComponent<GUITexture>().pixelInset = new Rect(x * spacingX,y * spacingY,128,128);
